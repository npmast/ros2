...
ros2 에서 다양한 프로그램밍 언어로 작성된 ROS2 노드들이 통신할 수 있도록 도와주는 핵심 공통 라이브러리  
C++로 작성된 핵심 기능을 기반으로 Python이나 다른 언어용 클라이언트 라이브러리가 공통된 API를 사용할 수 있다.  
노드 생성, 토픽 발행/구독, 서비스, 액션 등 ROS2의 핵심 기능들을 API 형태로 제공한다.  
rcl는 ROS2의 여러 언어 구현체가 공통적으로 의존하는 기반 기술이라고 볼 수 있다.  
## 워크스페이스와 패키지
...
빌드가 정상적으로 되면 build, install, log 폴더가 만들어 진다.  
...
$ ros2 pkg create test_package --build-type ament_python --dependencies rclpy std_msgs  
  ...
  * --dependencies rclpy std_msgs: ROS2 Python 노드와 문자열 메시지 사용
#### Turtlesim: server 버전인 경우 설치한다.
$ sudo apt install ros-jazzy-turtlesim  
### 5. ROS2 명령어  
노드 실행 명령어: ros2 run <packages name> <node name>  
